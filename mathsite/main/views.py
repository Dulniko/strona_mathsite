...
def ranking(request):
    ...
    rk10 = RankingTo10.objects.all().order_by(sort_by)
    
    filter_by = request.POST.get('filter_by', '')

    if filter_by:
        rk10 = RankingTo10.objects.filter(PlayerName=request.user.username).order_by(sort_by)
    else:
        rk10 = RankingTo10.objects.all().order_by(sort_by)
    return render(request, "main/ranking.html", {"rk10":rk10})
